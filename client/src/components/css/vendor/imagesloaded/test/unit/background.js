QUnit.test('background', function (assert) {
  ...
  assert.equal(imgLoad1.images.length, 1, '1 images on .images');
  ...
  imgLoad3.on('progress', function (instance, image/*, element */) {
    ...
    assert.equal(image.isLoaded, true, 'images.isLoaded == true');
    done();
  });
  ...
  imgLoad4.on('progress', function (instance, image) {
    assert.equal(image.isLoaded, true, 'images is loaded');
    done();
  });
